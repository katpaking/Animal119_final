...
<body>
	<div class="container mt-3">
		<img src="<%=contextPath%>/image/together.png" alt="img" class="d-block" height="60">
		<br/>
			...
			<tr>
				...
				<tbody>				   
					...
					<tr align="center">
						...
						<td>
							<a id="noUnderLine" href="<%=notWithFormTag%>suDetail&no=${bean.no}">${bean.subject}</a>
						</td>
						<td>${bean.regdate}</td>
						<td>
							<c:if test="${whologin == 2}">
							<a id="noUnderLine" href="<%=notWithFormTag%>suUpdate&no=${bean.no}${requestScope.pageInfo.flowParameter}">수정</a>
							</c:if>
						</td>
						<td>
							<c:if test="${whologin == 2}">
								<a id="noUnderLine" href="<%=notWithFormTag%>suDelete&no=${bean.no}${requestScope.pageInfo.flowParameter}">삭제</a>
							</c:if>
						</td>
					</tr>
					</c:forEach>				 
