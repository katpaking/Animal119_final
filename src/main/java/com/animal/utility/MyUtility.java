...
import com.animal.controller.SuperController;
...
public class MyUtility {
	public static void deleteOldImageFile(String realPath, MultipartRequest mr) {
		...
		//String[] deleteImage = mr.getParameterValues("deleteImage");
		String[] deleteImage = 
			{ 
	...
	public static MultipartRequest getMultipartRequest(HttpServletRequest request, String realPath) {
		// 이미지 업로드를 위한 멀티파트 객체를 생성 후 반환합니다.
		MultipartRequest mr = null; // 파일 업로드 객체
		int maxPostSize = 10 * 1024 * 1024; // 업로드 최대 용량
		final String ENCODING = "UTF-8"; // 인코딩 문자열

		try {
			mr = new MultipartRequest(request, realPath, maxPostSize, ENCODING, new DefaultFileRenamePolicy());
	...
	public static String getCurrentTime() {
		...
		return new SimpleDateFormat(pattern).format(new Date()) + " ";// 한칸 띄어쓰기.
	}
